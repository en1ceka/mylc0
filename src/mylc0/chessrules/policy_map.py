"""Lc0 policy move encoding (the 1858-move policy space).

The move list below is copied verbatim from ``kMoveStrs`` in
``lc0/src/neural/encoder.cc``; the upstream order matters because it *is* the
policy index. All moves are written from the point of view of the side to move
("white" orientation), which is how Lc0 stores moves internally: for a black
move the board -- and therefore the move -- is vertically flipped first.

Two mappings live here:

* ``MOVE_TO_IDX`` / ``IDX_TO_MOVE`` -- the 1858 policy indices themselves.
* ``ATTENTION_POLICY_GATHER`` -- the gather map that turns the attention policy
  head's 64*64 + 8*24 = 4288 raw logits into the 1858 policy vector. It is
  rebuilt from the move list here and was verified element-by-element against
  ``_policy_map`` in
  ``lczero-training/src/lczero_training/model/policy_head.py``.

Square indexing is Lc0's (and python-chess's): ``a1 = 0 ... h8 = 63``,
``index = rank * 8 + file``.
"""
...
# Transform flags, matching lc0/src/neural/encoder.h.
NO_TRANSFORM = 0
FLIP_TRANSFORM = 1       # mirror files (a<->h), i.e. ReverseBitsInBytes
MIRROR_TRANSFORM = 2     # mirror ranks (1<->8), i.e. ReverseBytesInBytes
TRANSPOSE_TRANSFORM = 4  # transpose along the a1-h8 diagonal

POLICY_SIZE = 1858
ATTENTION_POLICY_LOGITS = 64 * 64 + 8 * 24  # 4288
...
assert len(MOVE_STRS) == POLICY_SIZE

MOVE_TO_IDX = {m: i for i, m in enumerate(MOVE_STRS)}
IDX_TO_MOVE = MOVE_STRS

_PROMO_ORDER = {"q": 0, "r": 1, "b": 2}
...
def _square_name(square: int) -> str:
    return "abcdefgh"[square & 7] + "12345678"[square >> 3]


def _build_attention_gather() -> np.ndarray:
    """Index map: 4288 attention-policy logits -> 1858 policy entries.

    Layout of the raw logit vector produced by the attention policy head:

    * ``[0, 4096)``    -- ``from_square * 64 + to_square`` (the QK matrix).
    * ``[4096, 4288)`` -- promotion offsets, ``from_file * 24 + to_file * 3 +
      promotion``, promotion in (queen, rook, bishop) order. Knight promotions
      are *not* here: they reuse the plain 64x64 entry, exactly as in Lc0.
    """
    gather = np.zeros(POLICY_SIZE, dtype=np.int64)
    for i, mv in enumerate(MOVE_STRS):
        f, t = _sq(mv[0:2]), _sq(mv[2:4])
        if len(mv) == 5:
            gather[i] = 4096 + (f % 8) * 24 + (t % 8) * 3 + _PROMO_ORDER[mv[4]]
        else:
            gather[i] = f * 64 + t
    return gather


ATTENTION_POLICY_GATHER = _build_attention_gather()


def _reverse_bits_in_bytes(v: int) -> int:
    """Mirror files (a<->h). Lc0: ReverseBitsInBytes."""
...
def _reverse_bytes_in_bytes(v: int) -> int:
    """Mirror ranks (1<->8). Lc0: ReverseBytesInBytes."""
    return int.from_bytes((v & 0xFFFFFFFFFFFFFFFF).to_bytes(8, "little"), "big")


def _transpose_bits_in_bytes(v: int) -> int:
    """Lc0's ``TransposeBitsInBytes`` (utils/bititer.h), transcribed.

    Despite the "A1 to H8" wording in the upstream comment this is the flip
    across the **a8-h1** diagonal: ``(rank, file) -> (7 - file, 7 - rank)``.
    That matches ``Transform(Square, TransposeTransform)`` in encoder.cc, which
    flips the rank and flops the file, and the two must agree or the policy
    indices would not line up with the transformed planes.
    """
    v &= 0xFFFFFFFFFFFFFFFF
    v = (((v & 0xAA00AA00AA00AA00) >> 9)
         | ((v & 0x0055005500550055) << 9)
         | (v & 0x55AA55AA55AA55AA))
    v = (((v & 0xCCCC0000CCCC0000) >> 18)
         | ((v & 0x0000333300003333) << 18)
         | (v & 0x3333CCCC3333CCCC))
    v = (((v & 0xF0F0F0F000000000) >> 36)
         | ((v & 0x000000000F0F0F0F) << 36)
         | (v & 0x0F0F0F0FF0F0F0F0))
    return v & 0xFFFFFFFFFFFFFFFF


def transform_bitboard(bb: int, transform: int) -> int:
    """Apply an Lc0 canonicalization transform to a bitboard."""
    if transform & FLIP_TRANSFORM:
        bb = _reverse_bits_in_bytes(bb)
    if transform & MIRROR_TRANSFORM:
        bb = _reverse_bytes_in_bytes(bb)
    if transform & TRANSPOSE_TRANSFORM:
        bb = _transpose_bits_in_bytes(bb)
    return bb


def transform_square(square: int, transform: int) -> int:
    """Square transform matching :func:`transform_bitboard` exactly.

    This is lc0 v0.31's ``Transform(BoardSquare, int)`` (chess/bitboard.cc):
    the three transforms are applied in sequence, and the transpose is the
    reflection ``(rank, file) -> (7 - file, 7 - rank)``.

    Note on upstream: after the Square/File/Rank refactor, lc0 master's
    ``Transform`` in neural/encoder.cc flips the rank and flops the file
    independently, which is a 180-degree rotation rather than that reflection,
    so it no longer agrees with ``TransposeBitsInBytes`` (which does reflect --
    its +-9 shifts move squares along the a1-h8 direction, which is what a
    reflection across the a8-h1 diagonal does). Planes and policy indices must
    describe the same geometry, so the consistent, pre-refactor definition is
    the one used here. See ARCHITECTURE.md, "Deliberate differences".
    """
    file_ = square & 7
    rank = square >> 3
    if transform & FLIP_TRANSFORM:
    ...
    if transform & MIRROR_TRANSFORM:
        rank = 7 - rank
    if transform & TRANSPOSE_TRANSFORM:
        rank, file_ = 7 - file_, 7 - rank
    return rank * 8 + file_


def invert_transform(transform: int) -> int:
    """Lc0 encoder.cc ``MoveFromNNIndex`` inverse-transform derivation."""
    if transform & TRANSPOSE_TRANSFORM:
...
def _build_transform_tables():
    """``TRANSFORMED_IDX[t][i]``: policy index of move ``i`` after transform t.

    ``INVERSE_TRANSFORMED_IDX[t][i]`` undoes it, mirroring how Lc0 applies the
    inverse transform when decoding a network output back into a move.

    Entries are ``-1`` where the transformed move falls outside the 1858-move
    space. That only happens for promotions under a rank-mirroring or
    transposing transform, and Lc0 never generates such a combination:
    ``ChooseTransform`` refuses everything but the file flip while any pawn is
    on the board (encoder.cc, "If there are any pawns only horizontal flip is
    valid").
    """
    fwd = np.full((8, POLICY_SIZE), -1, dtype=np.int32)
    inv = np.full((8, POLICY_SIZE), -1, dtype=np.int32)
    for transform in range(8):
        itr = invert_transform(transform)
        for idx, mv in enumerate(MOVE_STRS):
            suffix = mv[4] if len(mv) == 5 else ""
            f = transform_square(_sq(mv[0:2]), transform)
            t = transform_square(_sq(mv[2:4]), transform)
            fwd[transform, idx] = MOVE_TO_IDX.get(
                _square_name(f) + _square_name(t) + suffix, -1)
            f = transform_square(_sq(mv[0:2]), itr)
            t = transform_square(_sq(mv[2:4]), itr)
            inv[transform, idx] = MOVE_TO_IDX.get(
                _square_name(f) + _square_name(t) + suffix, -1)
    return fwd, inv


TRANSFORMED_IDX, INVERSE_TRANSFORMED_IDX = _build_transform_tables()


# Fast path: lc0's ``kPackedIdxToNNIdx``. Index a move as
# ``promotion * 4096 + from * 64 + to`` with promotion in
# (none/knight, queen, rook, bishop) order, mirroring ``MoveAsPackedInt``.
_PROMO_SLOT = {"q": 1, "r": 2, "b": 3}
# python-chess piece types: KNIGHT 2, BISHOP 3, ROOK 4, QUEEN 5.
PROMOTION_SLOT = {None: 0, 2: 0, 3: 3, 4: 2, 5: 1}


def _build_packed_table() -> np.ndarray:
    table = np.full(4 * 64 * 64, -1, dtype=np.int32)
    for i, mv in enumerate(MOVE_STRS):
        f, t = _sq(mv[0:2]), _sq(mv[2:4])
        slot = _PROMO_SLOT[mv[4]] if len(mv) == 5 else 0
        table[slot * 4096 + f * 64 + t] = i
    return table


PACKED_TO_IDX = _build_packed_table()

# TRANSFORM_SQUARES[t][sq] == transform_square(sq, t)
TRANSFORM_SQUARES = np.array(
    [[transform_square(sq, t) for sq in range(64)] for t in range(8)],
    dtype=np.int32)


def policy_indices(moves, flipped: bool, transform: int = NO_TRANSFORM
                   ) -> np.ndarray:
    """Policy slots of ``moves`` (python-chess Moves), in Lc0's convention.

    ``flipped`` mirrors the move vertically for a black-to-move position, the
    same ``move.Flip()`` lc0 applies before indexing.
    """
    out = np.empty(len(moves), dtype=np.int64)
    table = PACKED_TO_IDX
    squares = TRANSFORM_SQUARES[transform] if transform else None
    for i, mv in enumerate(moves):
        ...
        if squares is not None:
            f = squares[f]
            t = squares[t]
        idx = table[PROMOTION_SLOT[mv.promotion] * 4096 + f * 64 + t]
        assert idx >= 0, f"move {mv.uci()} has no policy slot"
        out[i] = idx
    return out


def move_to_nn_index(uci: str, transform: int = NO_TRANSFORM) -> int:
    """Lc0 ``MoveToNNIndex``.

    ``uci`` must already be in "our" orientation (vertically flipped when black
    is to move). Knight promotions are encoded without the promotion suffix,
    exactly like ``MoveAsPackedInt`` does: its ``kPromotion`` bit is only set
    for queen/rook/bishop promotions.
    """
    ...
    if transform == NO_TRANSFORM:
        return idx
    out = int(TRANSFORMED_IDX[transform, idx])
    assert out >= 0, f"move {uci} has no policy slot under transform {transform}"
    return out


def move_from_nn_index(idx: int, transform: int = NO_TRANSFORM) -> str:
    """Lc0 ``MoveFromNNIndex`` (returns "our"-orientation UCI)."""
    if transform == NO_TRANSFORM:
        return MOVE_STRS[idx]
    out = int(INVERSE_TRANSFORMED_IDX[transform, idx])
    assert out >= 0, f"policy index {idx} is invalid under transform {transform}"
    return MOVE_STRS[out]
